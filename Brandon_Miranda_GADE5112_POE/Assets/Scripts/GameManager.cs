...
class GameManager : MonoBehaviour
{
    ...
    public void Start()
    {
       ...
       // SpawnWizardBuilding();
        ...
        Debug.Log(manager.Buildings.ToString());
        Debug.Log(manager.Units.ToString());
    ...
    void SpawnGrass()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            for (int k = 0; k < 20; k++)
            {
                ...
                GameObject spawnTilePrefab = Instantiate(tilePrefabs[0]) as GameObject;
                spawnTilePrefab.transform.position = pos;
            }
        }
    }
    ...
    void SpawnBuildings()
    {
        ...
        {
            if (building.Faction == A_TEAM)
            {
                if (building.Symbol == '~')
                {
                    //GameObject spawnTilePrefab;
                    GameObject spawnTilePrefab;
                    int x = building.X * 10;
                    int y = building.Y * 10;
                    Vector3 pos = new Vector3(x, 0, y);
                    spawnTilePrefab = Instantiate(tilePrefabs[1]) as GameObject;
                    spawnTilePrefab.transform.position = pos;
                }
                else if (building.Symbol == '^')
                {
                    ...
                    int y = building.Y * 10;
                    Vector3 pos = new Vector3(x, 0, y);
                    spawnTilePrefab = Instantiate(tilePrefabs[4]) as GameObject;
                    spawnTilePrefab.transform.position = pos;
            ...
            else if (building.Faction == B_TEAM)
            {
                if (building.Symbol == '~')
                {
                    //GameObject spawnTilePrefab;
                    ...
                    int y = building.Y * 10;
                    Vector3 pos = new Vector3(x, 0, y);
                    spawnTilePrefab = Instantiate(tilePrefabs[2]) as GameObject;
                    spawnTilePrefab.transform.position = pos;
                }
                else if (building.Symbol == '^')
                {
                    ...
                    int y = building.Y * 10;
                    Vector3 pos = new Vector3(x, 0, y);
                    spawnTilePrefab = Instantiate(tilePrefabs[5]) as GameObject;
                    spawnTilePrefab.transform.position = pos;

                }

            }
            else if (building.Faction == WIZARDS)
            {
                if (building.Symbol == '~')
                {
                    //GameObject spawnTilePrefab;
                    ...
                    int y = building.Y * 10;
                    Vector3 pos = new Vector3(x, 0, y);
                    spawnTilePrefab = Instantiate(tilePrefabs[3]) as GameObject;
                    spawnTilePrefab.transform.position = pos;
                }
            }

            //manager.Buildings[1].Symbol
            //manager.Buildings[1].X;
            ////GameObject spawnTilePrefab;
            //if (tileindex == 1)
            //{


            //    spawnTilePrefab = Instantiate(tilePrefabs[0]) as GameObject;
            //}

            //spawnTilePrefab.transform.position = pos;

        }
        //void SpawnWizardBuilding()
        //{
        //    {
        //        foreach (Building building in manager.Buildings)
        ...
        //                    int y = building.Y * 10;
        //                    Vector3 pos = new Vector3(x, 0, y);
        //                    spawnTilePrefab = Instantiate(tilePrefabs[6]) as GameObject;
        //                    spawnTilePrefab.transform.position = pos;
            ...
            foreach (Unit unit in manager.Units)
            {
                if (unit.Faction == A_TEAM)
                {
                    ...
                    if (unit.Symbol == '#')
                    {
                        ...
                        int y = unit.Y * 10;
                        Vector3 pos = new Vector3(x, 0, y);
                        spawnTilePrefab = Instantiate(unitPrefabs[0]) as GameObject;
                        spawnTilePrefab.transform.position = pos;
                    }
    ...
    void UseResourcesFromFaction(string faction, int usedResources)
    {
        foreach (Building building in manager.GetBuildingsByFaction(faction))
        {
            if (building is ResourceBuilding && !building.IsDestroyed)
            {
                ...
                int resourcesToUse = Mathf.Min(usedResources, resourceBuilding.Generated);
    ...
    private void CreateUnitsAndBuildings()
    {
        foreach (string faction in factions)
        {
            ...
            if (faction != WIZARDS)
            {
                AddUnits(Random.Range(5, 10), faction);
                AddBuildings(Random.Range(10, 15), faction);
            }
            else
            {
                AddUnits(Random.Range(5, 10), faction);
